...
public class OrderController {
    ...
    public ResultVo create(@Valid OrderVo orderVo, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            ...
            throw new OrderException(Constant.ERR_PARAMETER_ORDER_MSG, Constant.ERR_PARAMETER_ORDER_CODE);
        }
        OrderDTO orderDTO = convert(orderVo);
        if(CollectionUtils.isEmpty(orderDTO.getOrderDetailList())){
            log.error("[订单为空]");
            throw new OrderException(Constant.NULL_PARAMETER_CAR_MSG,Constant.NULL_PARAMETER_CAR_CODE);
        }
